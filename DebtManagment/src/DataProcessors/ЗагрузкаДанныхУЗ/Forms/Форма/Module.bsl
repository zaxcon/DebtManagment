	...
	Для Каждого Реквизит из ТаблицаРеквизитов цикл
		Если Реквизит.Вид="Адрес" тогда
			Для Каждого СтрокаДанных из ТаблицаДанных цикл
				...
				//СтрокаДанных.ПолучитьИдентификатор()
			КонецЦикла;	
		КонецЕсли;	
	КонецЦикла;	
...
//СтрокаТч.Тип =Перечисления.ТипыКонтактнойИнформации.Телефон; СтрокаТч.Вид = Справочники.ВидыКонтактнойИнформации.МобильныйТелефонДолжника;
//СтрокаТч.Тип =Перечисления.ТипыКонтактнойИнформации.Телефон; СтрокаТч.Вид = Справочники.ВидыКонтактнойИнформации.ДомашнийТелефонДолжника;
//СтрокаТч.Тип =Перечисления.ТипыКонтактнойИнформации.Телефон; СтрокаТч.Вид = Справочники.ВидыКонтактнойИнформации.РабочийТелефонДолжника;
//СтрокаТч.Тип =Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты; СтрокаТч.Вид = Справочники.ВидыКонтактнойИнформации.EmailДолжника;
...
Процедура АнализНаСервере()  
	Для Каждого СтрокаДанных Из ТаблицаДанных цикл 
	 ...
	 //    если Реквизит.Вид="Адрес" тогда
	 //   	 Для Каждого СтрокаДанных Из ТаблицаДанных цикл 
	 //   		 Если   СокрЛП(СтрокаДанных[Реквизит.Реквизит])<>"" тогда
	 //   			 //Сообщить(СтрокаДанных[Реквизит.Реквизит]); 
	 //   			 массивАдресов = ДобавитьАдресДляАнализа(массивАдресов, СтрокаДанных.ПолучитьИдентификатор(),СтрокаДанных[Реквизит.Реквизит]);
	 //   		 КонецЕсли;
	 //   	 КонецЦикла;
	 //   	 //Сообщить(массивАдресов);
	 ...
	 //   			//Сообщить(Соотв);
	 //   			НСтрока   =  ТаблицаДанных.НайтиПоИдентификатору(Соотв.Получить("id"));
	 //   			Если Соотв.Получить("validation-code") = "VALIDATED" тогда
	 //   			 	ПроверитьОбработатьРегионНаселенныйПукт(Соотв);
	 //   				 АдресПочта = СформироватьАдресПочты(Соотв);
	 //   				 //Сообщить(АдресПочта);
	 //   			 
	 //   				НСтрока[Реквизит.Реквизит]=АдресПочта;
	 //   			иначе
	 //   			НСтрока[Реквизит.Реквизит+"_Ошибка"]=Соотв.Получить("validation-code")+Символы.ПС+Соотв.Получить("quality-code");	
	 //   			Конецесли;	
	 //           КонецЦикла;
	 //   	КонецЕсли;
	 //   	
	 //   	////ПопыткаПривестиАдресКФИАС 
	 //   	Для Каждого СтрокаДанных Из ТаблицаДанных цикл 
	 //   		Если   СокрЛП(СтрокаДанных[Реквизит.Реквизит])<>"" и СокрЛП(СтрокаДанных[Реквизит.Реквизит+"_Ошибка"])="" тогда
	 //   			ЗначениеФИАС=ПопыткаПривестиПочтовыйАдресКФИАС(СтрокаДанных[Реквизит.Реквизит]);
	 //   			Если ЗначениеФИАС="" тогда
	 //   			 СтрокаДанных[Реквизит.Реквизит+"_Ошибка"]="Ошибка приведения адреса к ФИАС";	
	 //   			КонецЕсли;	
	 //   		КонецЕсли;
	 //   	КонецЦикла;
...
Процедура СоздатьРеестр(Команда)
	...
	Если СсылкаНаРеестр<>Неопределено тогда
		ОткрытьЗначение(СсылкаНаРеестр);
	КонецЕсли;	
КонецПроцедуры
...
Функция ЗаполнитьАдресВДанных(СтрокаДанныхАдреса,ИмяТаблицы)
	 ...
	 Если СтрокаДанных<>Неопределено тогда 
		 СтрокаДанных[ИмяРеквизита]=СтрокаДанныхАдреса.АдресПочта;
	КонецЕсли;	 
 КонецФункции

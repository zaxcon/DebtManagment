...
//	ИспользуемаяТаблица
	...
	//1 
		  ...
		  Группа = Элементы.Добавить(ИмяРеквизита+"_ГруппаОписание",Тип("ГруппаФормы"),ТаблицаПолейВыбора);
		  Группа.Вид = ВидГруппыФормы.ГруппаКолонок;
		  НоваяКолонка = Элементы.Добавить(ИмяРеквизита+"_СтатусВалидацииОписание", Тип("ПолеФормы"), Группа);
		  НоваяКолонка.ПутьКДанным = "ТаблицаАдреса_"+ИмяРеквизита+".СтатусВалидацииОписание";  
		  НоваяКолонка.Вид = ВидПоляФормы.ПолеВвода; 
		  НоваяКолонка.РежимРедактирования = РежимРедактированияКолонки.ВходПриВводе;  
		  
		    НоваяКолонка = Элементы.Добавить(ИмяРеквизита+"_КачествоРаспознаванияОписание", Тип("ПолеФормы"), Группа);
		  НоваяКолонка.ПутьКДанным = "ТаблицаАдреса_"+ИмяРеквизита+".КачествоРаспознаванияОписание";  
		  НоваяКолонка.Вид = ВидПоляФормы.ПолеВвода; 
		  НоваяКолонка.РежимРедактирования = РежимРедактированияКолонки.ВходПриВводе; 
		 ...
		 Если Результат.addressType="Административно-территориальный" тогда
			 ...
			 Если ПроверитьЗначениеАдресаПоФИАС( ДопПарметры.СтрокаДанных.Значение)  тогда
				 ...
				 ДопПарметры.СтрокаДанных.ОписаниеОшибки="";                
                  ...
                  ИмяРеквизита = ПолучитьИмяПоляПоИмениПоля(ДопПарметры.ИмяПоля);  
...
Процедура Загрузить(Команда)
    ...
    ЗагрузкаКодаПроверкиОшибок();
	ЗагрузкаФайлаБыстрая(ПутьКФайлу);
    ЗаполнитьНеобработанныеАдреса();
КонецПроцедуры
 
 &НаСервере
Процедура ЗагрузкаКодаПроверкиОшибок()
	 МассивДобавляемыхРеквизитов = Новый Массив;        
	 ОписаниеСоставногоТипа = Новый ОписаниеТипов("Строка");
	 МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("КодПроверкиОшибок",ОписаниеСоставногоТипа,"ТаблицаРеквизитов" ,"КодПроверкиОшибок"));
	 ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);
	 Для Каждого СтрокаРеквизитов из ТаблицаРеквизитов цикл
	 	Если ЗначениеЗаполнено(СтрокаРеквизитов.ОбработкаПроверки) тогда
	 		СтрокаРеквизитов.КодПроверкиОшибок = ПолучитьКодОбработкиНаСервере(СтрокаРеквизитов.ОбработкаПроверки);
	 	КонецЕсли;	
	 КонецЦикла;	
КонецПРоцедуры	

&НаКлиенте
Функция ЗагрузкаФайлаБыстрая(ИмяФайла)
ЗагрузитьДанныеВТаблицуДанных(ПрочитатьЛистExcel(ИмяФайла));
КонецФункции	
...
&НаСервере
Функция ЗагрузитьДанныеВТаблицуДанных(МассивДанныхEXEL)
	ДобавитьСтроки=Истина;
	Для Каждого Реквизит из ТаблицаРеквизитов цикл
		Если Реквизит.НомерКолонки>0 тогда
			ТекСтрока=0;
			Для ш=ПерваяСтрока-1 по МассивДанныхEXEL[Реквизит.НомерКолонки-1].Количество()-1 цикл
				Если ДобавитьСтроки Тогда
				НоваяСтрокаДанных =ТаблицаДанных.добавить();
				иначе
				НоваяСтрокаДанных=ТаблицаДанных[ТекСтрока];	
				КонецЕсли;
				НоваяСтрокаДанных[Реквизит.Реквизит+"_Загрузка"] = МассивДанныхEXEL[Реквизит.НомерКолонки-1][ш] ;
...
Функция ВернутьЗначениеДаты(СтрокаДата)
	Попытка
	Возврат СтроковыеФункцииКлиентСервер.СтрокаВДату(СтрокаДата);
исключение  
	Возврат Дата(1,1,1);
	КонецПопытки;
КонецФункции
...
Процедура ПроверкаОшибок(ВыводитьОшибки)          
 ...
 Процедура ПроверкаОшибокСтроки(СтрокаДанных,МассивСтрокПроверок,ВыводитьОшибки=Ложь)
	 ...
	 Для Каждого ЭлементПроверки из МассивСтрокПроверок цикл
		 ...
		 КонецЕсли;		 
		 //===========================МЕДЛЕННО
		// ПолучитьКодОбработкиНаСервере(ЭлементПроверки.ОбработкаПроверки);
		 Выполнить(ЭлементПроверки.КодПроверкиОшибок);
...
&НаСервере   
   Функция НайтиДолги(СтрокаДанных)
   	...
   	Если ОбщийМодульПользовательский.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных,"idДолг")  тогда
   		ДолгиСписок =НайтиДолгиПоИД(СтрокаДанных.idДолг);
   			Если ДолгиСписок.Количество()>0 тогда
		//	Сообщить("Нашли по ид долга");
			Возврат ДолгиСписок;			
		КонецЕсли;			
   	КонецЕсли;
	  ...
	  //Если есть должник ищем по нему
	   ...
	   КонецЕсли;                     
	    //Есули Не нашли или это новый ищем долги уже прогруженные выше
	   Если ДолгиСписок.Количество()=0 тогда
...
&НаСервере
Функция НайтиДолгиПоИД(idДолг)
	МассивДолгов =Новый Массив();
	Если (idДолг=0)тогда
	 	Возврат МассивДолгов;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("idДолг", idДолг);
...
Функция НайтиДолжников(СтрокаДанных)
	//Если есть ИД, то ищем в первую очередь по нему
	Если ОбщийМодульПользовательский.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных,"idДолжник")  тогда
		ДолжникСписок =НайтиДолжникаИД(СтрокаДанных.idДолжник);
		Если ДолжникСписок.Количество()>0 тогда
		//	Сообщить("Нашли по ид");
...
&НаСервере
Функция НайтиДолжникаИД(idДолжник)
	МассивДолжников =Новый Массив();
	Если (idДолжник=0)тогда
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	Должники.idДолжник = &idДолжник";
	
	Запрос.УстановитьПараметр("idДолжник", idДолжник);
	РезультатЗапроса = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
...
&НаСервере
Процедура СоздатьОбъектыНаСервереНовый()
	// Вставить содержимое обработчика. 
	ТЗРеквизиты =РеквизитФормыВЗначение("ТаблицаРеквизитов");
	ТЗРеквизиты.Свернуть("ОбъектДанных");
	СписокСоздаваемыхОбъектов =ТЗРеквизиты.ВыгрузитьКолонку("ОбъектДанных"); 
...
&НаСервере
Функция ВернутьСоответствиеОписаниеНормализацииАдреса()
	НачСоответсвие = Новый Соответствие;
	НачСоответсвие.Вставить("GOOD"				,"Пригоден для почтовой рассылки");
	НачСоответсвие.Вставить("ON_DEMAND"				,"До востребования");
	НачСоответсвие.Вставить("POSTAL_BOX"				,"Абонентский ящик");
	НачСоответсвие.Вставить("UNDEF_01"				,"Не определен регион");
	НачСоответсвие.Вставить("UNDEF_02"				,"Не определен город или населенный пункт");
	НачСоответсвие.Вставить("UNDEF_03"				,"Не определена улица");
	НачСоответсвие.Вставить("UNDEF_04"				,"Не определен номер дома");
	НачСоответсвие.Вставить("UNDEF_05"				,"Не определена квартира/офис");
	НачСоответсвие.Вставить("UNDEF_06"				,"Не определен");
	НачСоответсвие.Вставить("UNDEF_07"				,"Иностранный адрес");

	Возврат НачСоответсвие; 			
КонецФункции	

&НаСервере
Функция ВернутьСоответствиеОписаниеАдреса()
	НачСоответсвие = Новый Соответствие;
	НачСоответсвие.Вставить("CONFIRMED_MANUALLY"				,"Подтверждено контролером");
	НачСоответсвие.Вставить("VALIDATED"				,"Уверенное распознавание");
	НачСоответсвие.Вставить("OVERRIDDEN"				,"Распознан: адрес был перезаписан в справочнике");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_UNPARSED_PARTS"				,"На проверку, неразобранные части");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_ASSUMPTION"				,"На проверку, нет основных частей");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_NO_MAIN_POINTS"				,"На проверку, неразобранные части");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_NUMBER_STREET_ASSUMPTION"				,"На проверку, предположение по улице");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_NO_KLADR_RECORD"				,"На проверку, нет в КЛАДР");
	НачСоответсвие.Вставить("NOT_VALIDATED_HOUSE_WITHOUT_STREET_OR_NP"				,"На проверку, нет улицы или населенного пункта");
	НачСоответсвие.Вставить("NOT_VALIDATED_HOUSE_EXTENSION_WITHOUT_HOUSEP"				,"На проверку, нет дома");
	НачСоответсвие.Вставить("NOT_VALIDATED_HAS_AMBI"				,"На проверку, неоднозначность");
	НачСоответсвие.Вставить("NOT_VALIDATED_EXCEDED_HOUSE_NUMBER"				,"На проверку, большой номер дома");
	НачСоответсвие.Вставить("NOT_VALIDATED_INCORRECT_HOUSE"				,"На проверку, некорректный дом");
	НачСоответсвие.Вставить("NOT_VALIDATED_INCORRECT_HOUSE_EXTENSION"				,"На проверку, некорректное расширение дома");
	НачСоответсвие.Вставить("NOT_VALIDATED_FOREIGN"				,"Иностранный адрес");	
	НачСоответсвие.Вставить("NOT_VALIDATED_DICTIONARY"				,"На проверку, не по справочнику");
	Возврат НачСоответсвие; 			
КонецФункции	
	...
	Если массивАдресов.Количество()>0 тогда
		...
		Для Каждого Соотв Из ПочтаОтвет Цикл
			...
			Для Каждого Эл Из Соотв Цикл
				...
				Конецесли;
			КонецЦикла;
			Стр.СтатусВалидацииОписание =ВернутьОписаниеКачестваРаспознания(Стр.СтатусВалидации);
				Стр.КачествоРаспознаванияОписание = ВернутьОписаниеНормализацииАдреса(Стр.КачествоРаспознавания);
...
&НаСервере
Функция ВернутьОписаниеКачестваРаспознания(КачествоРаспознавания)
	СоответствиеОшибки = ВернутьСоответствиеОписаниеАдреса();	
	Описание = СоответствиеОшибки.Получить(КачествоРаспознавания);
	Если Описание = Неопределено тогда
		Описание =КачествоРаспознавания;
	КонецЕсли;
		Возврат Описание;
КонецФункции	

&НаСервере
Функция ВернутьОписаниеНормализацииАдреса(КодНормализацииРаспознавания)
	СоответствиеОшибки = ВернутьСоответствиеОписаниеНормализацииАдреса();	
	Описание = СоответствиеОшибки.Получить(КодНормализацииРаспознавания);
	Если Описание = Неопределено тогда
		Описание =КодНормализацииРаспознавания;
	КонецЕсли;
		Возврат Описание;
КонецФункции	
...
КонецФункции	
 ...
 &НаСервере 
Функция ДополнитьПреобразоватьСтрокуКТелефону(СтрокаТелефон)
	
Индекс=1; 
	ТелефонИтог="";
	Пока Индекс<=СтрДлина(СтрокаТелефон) цикл
		 ТелефонИтог = ТелефонИтог +?(Найти("0123456789",Сред(СтрокаТелефон,Индекс,1)),Сред(СтрокаТелефон,Индекс,1),"");
		 Индекс=Индекс+1;
	КонецЦикла;
		
Если (СтрДлина(ТелефонИтог)=11 и ((Сред(ТелефонИтог,0,1)="8") или (Сред(ТелефонИтог,0,1)="7"))) тогда
	ТелефонИтог="7"+Сред(ТелефонИтог, 2);
	Возврат ТелефонИтог;
КонецЕсли;	

Если (СтрДлина(ТелефонИтог)=10 )тогда
	ТелефонИтог="7"+ТелефонИтог;
	Возврат ТелефонИтог;
КонецЕсли;	

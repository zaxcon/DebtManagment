...
&НаКлиенте
Процедура СписокДолгиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			ПоказатьЗначение(,Элемент.ТекущиеДанные.Долг );
		КонецЕсли;	
КонецПроцедуры
...
&НаКлиенте
	 ...
	 Процедура УстановитьОтборПоТекущемуДолгу(Долг)
		 ...
		 ИтогоСуммаПлатежей=ПолучитьСуммуПлатежейПоДолгу(Долг);
	 КонецПроцедуры 
	 
	 &НаСервере
	 Функция ПолучитьСуммуПлатежейПоДолгу(Долг)
	 	//{{КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
	 	// Данный фрагмент построен конструктором.
	 	// При повторном использовании конструктора, внесенные вручную изменения будут утеряны!
	 	...
	 	Запрос.Текст =
	 		"ВЫБРАТЬ
	 		|	СУММА(ОстаткиДолгов.Сумма) КАК СуммаПлатежа
	 		|ИЗ
	 		|	РегистрНакопления.ОстаткиДолгов КАК ОстаткиДолгов
	 		|ГДЕ
	 		|	ОстаткиДолгов.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	 		|	И ОстаткиДолгов.Долг = &Долг";
	 	
	 	Запрос.УстановитьПараметр("Долг", Долг);
	 	
	 	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	 	РезультатЗапроса.Следующий();
	 	Возврат РезультатЗапроса.СуммаПлатежа
...
&НаКлиенте
		...
		иначе
	     ...
	     КонецЕсли;
			...
			ОписаниеОповещенияОЗакрытии =НОвый ОписаниеОповещения("ОповоеститьОДобавленийПлатежа",ЭтотОбъект);
			
			ОткрытьФорму("Документ.Платеж.Форма.ФормаДокументаОднострочная",ПараметрыПлатежа,,,,,ОписаниеОповещенияОЗакрытии);
		КонецПроцедуры

 &НаКлиенте
 Процедура ОповоеститьОДобавленийПлатежа(Результат,ДопПарметры) Экспорт  
	СписокДолгиПриАктивизацииСтроки(Элементы.СписокДолги);
КонецПроцедуры

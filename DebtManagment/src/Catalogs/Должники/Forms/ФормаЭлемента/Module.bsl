...
Процедура СписокМессенджерыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
			...
			Оповещение= НОвый ОписаниеОповещения("ОбработатьДобавлениеМессенджера",ЭтотОбъект,ДопПараметры); 
			ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ФормаВводСоцсети",Новый Структура("ПараметрыЗаполнения",ПолучитьСтруктуруПараметровМессенджер()),Элемент,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
